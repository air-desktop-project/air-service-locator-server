...
| Descriptions d'appareils, points de poussée | **Oui** | Une autorisation accordée chez l'une réveille les appareils du bénéficiaire depuis celle-là (décision 9) ; le point doit être là où la notification part. La description suit l'appareil qu'elle décrit. Ce qu'une racine apprend d'un point à l'envoi — qu'il est mort, qu'il a trop reçu — ne se réplique pas (décision 27). |
...
| **Domaines** — propriétaire, alias, délégués, hébergeur ; **le rattachement des machines** (2026-09-26) | **Oui** | Écrits aux racines depuis les applications (`annuaires.md` §2 bis) : ce sont des décisions du compte, et l'une ou l'autre racine les reçoit selon le tirage de l'alias. |
| **Inscriptions d'annuaires locaux**, leur code, leur décision, leur retrait ; **le groupe des administrateurs** (2026-09-26) | **Oui** | Un administrateur accepte chez l'une une inscription déclarée chez l'autre ; l'annuaire local se connecte aux deux, et les deux doivent le reconnaître. |
...
| **Les domaines** (2026-09-26, proposé dans le détail) — un alias posé des deux côtés ; une machine rattachée à deux domaines des deux côtés ; un hébergeur changé des deux côtés | **Le plus récent gagne** : l'alias de domaine n'est pas unique (`modele.md` §2.11), il se remplace comme un nom ; le rattachement d'une machine est UN champ de la machine, et le dernier dit où elle est ; l'hébergeur aussi. | Un alias, un déplacement ou une bascule d'hébergeur écrits dans la fenêtre. Ils se réécrivent. |
| **Une délégation retirée d'un côté, utilisée de l'autre** — le délégué rattache sa machine pendant que le propriétaire lui retire la main | **Le retrait l'emporte toujours**, comme une révocation — et il détache les machines du délégué (proposé), quel que soit l'ordre d'arrivée. | Un rattachement fait dans la fenêtre. |
| **Une inscription acceptée d'un côté, refusée de l'autre** — deux administrateurs, deux racines, la même minute | **Le refus l'emporte** (proposé). Accepter donne aux racines une parole à répéter ; refuser ne coûte qu'une nouvelle demande. Dans le doute, on ne sert pas. **Une décision est donc définitive pour CETTE inscription** : un refus, où qu'il arrive, l'emporte sur toute acceptation, dans tous les ordres ; redemander, c'est déclarer une nouvelle inscription, avec un nouveau code. | Une acceptation, qui se redemande par une nouvelle inscription. |
...
| **Le dernier domaine d'un compte supprimé d'un côté pendant qu'un autre l'est de l'autre** | Chaque racine refuse localement de supprimer le dernier ; mais deux suppressions de deux domaines différents, chacune acceptée là où il en restait deux, laisseraient le compte sans domaine. **« Refuser la seconde arrivée » dépendrait de l'ordre, et les deux racines garderaient chacune un domaine différent** — l'écueil exact de §3.1. La règle est donc une fonction de l'ENSEMBLE des suppressions : **si elles laissaient le compte sans domaine, celui des domaines supprimés dont l'estampille de création est la plus petite reste**, et sa suppression est sans effet. Les deux racines calculent le même survivant, dans tous les ordres (proposé). | Une suppression, qui se refera ; l'application relit ses domaines après un `DELETE` et voit lequel est resté. |
...
**Les genres du 2026-09-26 — les domaines** (proposés ; les numéros se fixeront
à la PR de code, après le genre 20) :

| Genre | Charge | Règle d'application |
|---|---|---|
| `domaine` | identifiant ‖ propriétaire | Insérer si absent. **Porté aussi par `POST /v1/comptes`** : le premier domaine voyage avec le compte. |
| `domaine-alias` | domaine ‖ alias NFC ou rien | Le plus récent. |
| `domaine-hebergeur` | domaine ‖ `racines` ou `n-…` | Le plus récent ; refusé si l'inscription nommée est retirée. |
| `domaine-supprime` | domaine | Marquer supprimé. Puis, si le compte n'a plus aucun domaine vivant et n'est pas effacé, **le plus ancien des domaines supprimés** — la plus petite estampille de création — est tenu vivant, et sa marque sans effet : une fonction de l'ensemble, pas de l'ordre (§3.1, §3.2). |
| `delegation` / `delegation-retiree` | domaine ‖ compte | Insérer si absente / **toujours**, et détacher les machines du délégué. |
| `machine-domaine` | machine ‖ domaine ou rien | Le plus récent — un champ de la machine, avec son estampille, comme le nom et les capacités. |
| `inscription` | `n-…` ‖ propriétaire ‖ adresse ‖ empreinte du code | Insérer si absente. |
| `inscription-liee` | `n-…` ‖ clé d'identité | La clé liée au code, comme `cle-machine`. |
| `inscription-decidee` | `n-…` ‖ accepte ou refuse ‖ administrateur | Le refus l'emporte (§3.2). |
| `inscription-retiree` | `n-…` | **Toujours** ; ferme la voie de l'annuaire local ici aussi ; ses domaines reviennent aux racines. |
| `administrateur` / `administrateur-retire` | compte | Insérer / **toujours**. |

**`compte-efface` en retire davantage** : ses domaines, les délégations
données et reçues, ses inscriptions ; les machines de ses délégués rattachées à
ses domaines sont **détachées**, pas retirées (`modele.md` §2.11).
...
opération à échanger (proposé, `modele.md` §2.11).
...
| 31 | **La délégation** : le propriétaire délègue la gestion d'un domaine à des comptes existants ; un délégué voit le domaine, rattache et détache SES machines, pose l'alias ; un seul niveau ; seul le propriétaire délègue. Rattacher ne donne aucun droit de lecture (C10). | **Décidé** (2026-09-26, Thierry) |
...
| 33 | **Le groupe des administrateurs des racines** (`modele.md` §2.12), seul groupe de la v1 ; premier membre le compte de Thierry ; membres nommés et retirés **sous la clé d'exploitant**, et sous elle seule. **Renverse** l'argument de la décision 26 — « pas de `u-…` qui vaut plus que les autres » — pour ce seul jugement ; un administrateur ne peut rien d'autre qu'accepter ou refuser une inscription. | **Décidé** (2026-09-26, Thierry) |
...
| 35 | **L'alias de domaine** : facultatif, **non unique**, UTF-8 de 1 à 64 octets aux règles du nom de machine, rangé en **NFC** ; recherche **exacte** après NFC et **pliage simple de casse** d'Unicode, réservée aux comptes authentifiés, sans préfixe ni énumération ; la réponse est la liste de tous les domaines qui le portent, avec l'annuaire qui fait autorité. Donnée publique choisie : exception de C13. Le plus récent gagne entre racines. | **Décidé** (2026-09-26, Thierry) — la version d'Unicode épinglée : proposé |
| 36 | **La voie de l'annuaire local** (`protocole.md` §3 ter) : l'annuaire local OUVRE vers chaque racine (il est derrière un NAT), prouve sa clé d'identité `n-…` comme une racine, reçoit les machines de ses domaines et leurs révocations, pousse ses services et leur état ; C11 vérifié à chaque cadre. Les domaines, les délégations, le rattachement et les inscriptions s'écrivent aux racines depuis les applications ; les services et leur état, chez l'annuaire local. | **Proposé** — la forme sur le fil et le partage de l'autorité, à confirmer |
