...
| `fuzz_asl_jwt` | `jwt` | La porte d'entrée de Play Integrity : des octets quelconques vers les trois segments d'un JWS. Elle éprouve que le découpage ne panique jamais, que `signe()` est bien `en-tête.charge` (ce qu'ES256 hachera), que le décodage base64url ne déborde pas du tampon et que ce qui se décode se ré-encode à l'identique. |
| `fuzz_asl_play` | `play` | L'ouverture d'un jeton Play Integrity sous des clés FIXES : déballage de clé (AES key wrap), déchiffrement authentifié (AES-GCM), signature (ES256). Aucun jeton tiré au hasard ne s'ouvre sous ces clés — sa propriété centrale est qu'un `Ok` serait une contrefaçon. Elle éprouve aussi que rien ne panique, que tout refus est nommé, et que l'ouverture est stable. |
