...
| `fuzz_asl_api_corps` | `corps` | Les corps de l'API mobile, et **le premier TEXTE LIBRE du produit** : un nom de machine accepte désormais l'UTF-8 entier. Tout relâchement d'un analyseur demande qu'on le pousse sur des octets qu'on n'a pas choisis. Aucun nom accepté ne porte de guillemet, de contrôle, ni de caractère qui change l'affichage de ses voisins. Et le **point de poussée** (2026-09-25) : une URL que l'annuaire APPELLERA — tout point accepté est `https://`, vers un nom DNS qui ne finit pas en étiquette numérique, sans identifiants ni fragment, sur le port 443, et se réécrit et se relit à l'identique. |
...
| `fuzz_asl_proto_liste` | `liste` | **La seule qui découpe sans comprendre.** `elements` suit les crochets, les accolades et les chaînes, et rend des tranches sans rien décoder — une faute n'y plante pas, elle coupe au mauvais endroit et rend deux moitiés qui se décodent en autre chose. Et c'est un CLIENT qui lit ces octets, dans les cinq liaisons. Elle vérifie que les tranches restent dans le tampon, dans l'ordre et sans se chevaucher, et que l'aller-retour est fidèle. |
...
| `fuzz_asl_registre_aller_retour` | `registre` | **La seule qui protège des octets DÉJÀ ÉCRITS.** Un codec réseau se rattrape à la connexion suivante ; celui-ci écrit sur un disque, et rend un enregistrement faux durablement. Le cas qui compte n'est pas le disque qui ment — c'est une version future relue par une version ancienne, ce qui arrive à chaque retour arrière de déploiement. |
| `fuzz_asl_store_application` | `application` | **LA SEULE DE L'ÉTAGE 3, ET C'EST DÉLIBÉRÉ.** L'entrepôt fait des entrées-sorties, sa couverture est hors de portée — mais l'APPLICATION des opérations, elle, se fuzze : un attaquant qui tient l'autre racine choisit ce qu'on applique, et aucune suite de cadres ne doit faire paniquer ni corrompre. L'entrepôt est EN MÉMOIRE (`Entrepot::en_memoire`), donc sans `fsync` : des milliers d'applications par seconde là où un disque en ferait quelques centaines. Elle éprouve que rien ne panique dans les deux modes — le flux avec ses gardes, l'instantané avec la règle de conflit —, et que l'instantané se relit intact après coup, ce qui prend en défaut tout index laissé en désaccord avec sa table. |
...
| `fuzz_asl_attest_objet` | `attestation` | **LA PLUS EXPOSÉE DU DÉPÔT** : ces octets viennent d'un appareil qui n'a PAS ENCORE DE COMPTE. Toutes les autres grammaires se lisent après qu'une identité a été établie ; celle-ci se lit avant, et c'est ce qu'elle sert à établir. Elle éprouve trois choses qu'un objet bien formé ne montre jamais : que le curseur AVANCE toujours (sans quoi un conteneur boucle indéfiniment), que tout ce qui est rendu est une tranche DE L'ENTRÉE (sans quoi le nonce se calculerait sur autre chose que ce qui a été signé), et que `authData` se reconstitue octet pour octet depuis ses morceaux. |
| `fuzz_asl_apple_verifier` | `apple` | **LA SEULE DONT UN `Ok` PEUT ÊTRE UNE CONTREFAÇON.** Sous la racine du banc, une seule clé a jamais été certifiée ; tout `Ok` doit rendre exactement celle-là, sinon libFuzzer a forgé une attestation pour une clé qu'aucune chaîne n'a signée. Elle part d'un objet ACCEPTÉ — c'est pourquoi la racine est celle du banc et non celle d'Apple, sous laquelle tout serait refusé à la chaîne et rien après ne serait exercé. |
| `fuzz_asl_keystore` | `keystore` | **LA SECONDE DONT UN `Ok` PEUT ÊTRE UNE CONTREFAÇON**, et la seule qui parte d'une capture RÉELLE : la chaîne du Fairphone 5 du 2026-09-16, sous la racine de Google. Sous cette racine, la seule feuille que libFuzzer tienne est celle de la capture ; tout `Ok` doit donc rendre exactement sa clé, sinon une chaîne a été forgée sans la clé de Google. Elle éprouve aussi, sur des octets quelconques, que la case du fil se découpe sans paniquer, que le lecteur de `KeyDescription` ne panique pas et ne rend que des tranches de l'entrée, que tout refus est nommé, et que le verdict est stable. |
| `fuzz_asl_reveil` | `reveil` | **LES SEULS OCTETS QUE L'ANNUAIRE LIT D'UN SERVEUR QU'IL N'A PAS CHOISI** : la ligne de statut d'un serveur de poussée désigné par l'utilisateur, et les adresses qu'un nom quelconque résout. Un code n'est rendu que d'une ligne `HTTP/1.x` entière, ce qui la suit ne change rien, une IPv4 enfouie (`::ffff:0:0/96`, `64:ff9b::/96`) se juge comme elle-même, et rien hors de `2000::/3` n'est admis. |
