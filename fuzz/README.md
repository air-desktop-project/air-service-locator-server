...
| `fuzz_asl_jwt` | `jwt` | Le découpage d'un JWS compact : des octets quelconques vers ses trois segments. Elle éprouve que le découpage ne panique jamais, que `signe()` est bien `en-tête.charge` (ce qu'ES256 hachera), que le décodage base64url ne déborde pas du tampon et que ce qui se décode se ré-encode à l'identique. `asl-jwt` ne sert plus rien depuis le retrait de Play Integrity (C19, 2026-09-16) ; elle reste, et sa cible avec elle. |
